...
// and then take ownership of the module that the function was compiled into.
...
public:
	...
	{
		// Move ownership of FnAST to a shared pointer -- c++11 lambdas don't
		// support capture-by-move, which is be required for unique_ptr.
		...
		// execution hits the compile callback(vid the stub).
		//
		...
		// The JIT runtime (the resolver block) will use the return address of this function
		// as the address to continue at once it has reset the CPU state to
		// what it was immediately before the call. 
		auto CompileAction = [this, SharedFnAST]() {
			...
			JITTargetAddress SymAddr = canFail(Sym.getAddress());
		...
		auto CCAddr = cantFail(CompileCallbackMgr->getCompileCallback(std::move(CompileAction)));
		
		// Create an indirect stub. This servers as the functions "canonical definition" - an
		// unchanging (constant address) entry point to the function implementation.
		// Initially we point the stub's function-pointer at the compile callback that we just
