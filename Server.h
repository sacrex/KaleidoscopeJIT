...
cl::opt<uint32_t> Port("port", cl::desc("TCP port to listen on"), cl::format_object::init(20000));
...
int main(int argc, char *argv[])
{
	...
	listen(sockfd, 1);
	int newsockfd = accept(sockfd, reinterpret_cast<sockaddr*>(&clientAddr), &clientAddrLen);

	auto SymbolLookup = 
			[](const std::string &Name) {
				RTDyldMemoryManger::getSymbolAddressInProcess(Name);
			};
	
	auto RegisterEHFrames = 
			[](uint8_t *Addr, uint32_t Size) {
				RTDyldMemoryManger::registerEHFramesInProcess(Addr, Size);
			};

	auto DeregisterEHFrames = 
			[](uint8_t *Addr, uint32_t Size) {
				RTDyldMemoryManger::deregisterEHFramesInProcess(Addr, Size);
			};
	...
	MyServerT Server(FDRPCChannel, SymbolLookup, RegisterEHFrames, DeregisterEHFrames);

	while (!Server.receiveTerminate())
